...
\begin{algorithm}[ht]
  ...
  $m \leftarrow K/t$ \Comment*[f]{number of examplers per class} \\
  \For {$y = 1..s-1$}{
  ...
  $\mathcal{P} \leftarrow \left(P_1, ..., P_t\right)$ \Comment*[f]{new exampler sets} \\
...
\begin{algorithm}[ht]
  ...
  \Require{$\mathcal{P} = \left( P_1, ..., P_{s-1}\right)$ \Comment*[f]{exampler sets}}
  \Require{$\Theta$ \Comment*[f]{ current model parameters}}
...
\begin{algorithm}[ht]
  ...
  \Input{$m$ target number of examplers}
